...
<script>
  export default {
    ...
    created () {
      this.$fetch('food', this.$qs.stringify({rows: 100})).then(rs => {
        this.foodList = rs.list
      })
      this.$fetch('member', this.$qs.stringify({member_id: this.$store.state.user_id})).then(rs => {
        if(rs.favorite){
    ...
    methods:{
      ...
      finish () {
        ...
        this.$fetch('member_favorite', this.$qs.stringify(req)).then(rs => {
          this.$toast('设置成功')
          setTimeout(()=>{
            this.onClickLeft()
          },1500)
